...
public class JpegEncoder extends Frame {
	...
	void WriteArray(byte[] data, OutputStream out) throws IOException {
		int length = (((int) (data[2] & 0xFF)) << 8) + (int) (data[3] & 0xFF)
				+ 2;
			...
			for (u = 0; u < 8; u++) {
				...
				output[v][u] *= ((double) (0.25)
						* ((u == 0) ? ((double) 1.0 / Math.sqrt(2))
								: (double) 1.0) * ((v == 0) ? ((double) 1.0 / Math
						.sqrt(2)) : (double) 1.0));
			}
		}
...
class Huffman {
	...
	public void initHuf() {
		...
		/*
		 * init of the DC values for the chrominance [][0] is the tcode [][1] is
		 * the number of bit
		 */
		p = 0;
		...
		/*
		 * Init of the AC hufmann tcode for the chrominance matrix [][][0] is
		 * the tcode & matrix[][][1] is the number of bit needed
		 */
		p = 0;
		...
		/*
		 * init of the DC values for the luminance [][0] is the tcode [][1] is
		 * the number of bit
		 */
		p = 0;
		...
		/*
		 * Init of the AC hufmann tcode for luminance matrix [][][0] is the
		 * tcode & matrix[][][1] is the number of bit
		 */
		p = 0;
